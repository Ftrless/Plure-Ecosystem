...
public class WarpTransfer implements Command<ServerCommandSource> {
    @Override
    ...
    public void exec(CommandContext<ServerCommandSource> context, ServerPlayerEntity senderPlayer,
        ...
        Message notExist = TextUtils.translation("cmd.warp.transfer.error.not-exist", FormatUtils.Colors.ERROR);
        Message notOwner = TextUtils.translation("cmd.warp.transfer.error.not-owner", FormatUtils.Colors.ERROR);
        Message selfTransfer = TextUtils.translation("cmd.warp.transfer.error.self-transfer", FormatUtils.Colors.ERROR);
        ...
        if (senderPlayer.equals(targetPlayer)) {
            throw CommandHelper.createException(selfTransfer);
        }

        warpData.owner = targetPlayer.getUuid();

        warpDataManager.editWarp(warpName, warpData);

        PlayerUtils.sendFeedback(context, "cmd.warp.transfer.feedback", warpName, targetPlayer.getName());
    }
}
